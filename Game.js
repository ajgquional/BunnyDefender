...
BunnyDefender.Game.prototype = {
	create: function() {
		...
		this.timer = this.time.create(false); // so that the timer will not "destroy" itself within the game
	...
	buildBunnies: function() {
		...
		// to build individual bunnies then add them to the group
		for(var i = 0; i < this.totalBunnies; i++) {
			// creates a bunny within the hill
			...
			b.body.moves = false;
			// rest state of bunny: frame 1-58 (in the texture atlas)
			b.animations.add('Rest', this.game.math.numberArray(1,58));
			// walk state of bunny: frame 68-107 (in the texture atlas)
			b.animations.add('Walk', this.game.math.numberArray(68,107));
			// for each of the bunny: rest state at 24 frames per second
	...
	assignBunnyMovement: function(b) {
		// bposition - position that the bunny wants to go
		bposition = Math.floor(this.rnd.realInRange(50, this.world.width-50));
		// bdelay - individual bunnies have different movement time
		// 2000 ms - 6000 ms (2s - 6s)
	...
	buildSpaceRocks: function() {
		...
		// creating each spacerock:
		for(var i = 0; i < this.totalSpacerocks; i++) {
			// positioning the spacerock:
			...
			// varying the size of the spacerocks:
			var scale = this.rnd.realInRange(0.3, 1.0);
			r.scale.x = scale;
			r.scale.y = scale;
			// enabling "gravity" physics:
	...
	buildEmitter:function() {
		...
		// describing the property of the emitter
	...
	checkBunniesLeft: function(){
		if(this.totalBunnies <= 0){
			...
			// game over message is placed at the center
